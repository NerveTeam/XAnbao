...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    message=[NSMutableArray arrayWithCapacity:0];
    
    messdef=[NSUserDefaults standardUserDefaults];
    [messdef synchronize];
...
//        }];
    [JPUSHService setTags:nil alias:[XABUserLogin getInstance].userInfo.mobile callbackSelector:@selector(tagsAliasCallback:tags:alias:) object:self];
...
//     navLogin = [[UINavigationController alloc] initWithRootViewController:tabBarController];
    
    
    
    navLogin=(UINavigationController *)tabBarController.viewControllers[tabBarController.selectedIndex];
...
- (void)jpushNotificationCenter:(UNUserNotificationCenter *)center willPresentNotification:(UNNotification *)notification withCompletionHandler:(void (^)(NSInteger))completionHandler {
    ...
    if([notification.request.trigger isKindOfClass:[UNPushNotificationTrigger class]]) {
        ...
        if ([typeStr isEqualToString:@"notice"]) {
            ...
            settingvc.type=typeStr;
            settingvc.jianzhangid=noticeid;
            settingvc.hidesBottomBarWhenPushed=YES;
            [navLogin pushViewController:settingvc animated:1];
            
            messageDic =@{@"alert":Alertstr ,@"url":urlStr ,@"type":typeStr,@"noticeid":noticeid};
            [message addObject:messageDic];
            
            [messdef setValue:message forKey:@"mess"];
        }
        
        if ([typeStr isEqualToString:@"classNotice"]) {
            XABArticleViewController *article = [[XABArticleViewController alloc]initWithUrl:urlStr];
            
            [navLogin pushViewController:article animated:1];
...
//            settingvc.url=urlStr;
            ...
            messageDic =@{@"alert":Alertstr ,@"url":@"" ,@"type":typeStr,@"jiazhangid":jiazhangid};
            [message addObject:messageDic];
