...
//    BOOL isHD = ([[UIScreen mainScreen] respondsToSelector:@selector(scale)])?(([[UIScreen mainScreen] scale] == 2)?YES:NO):NO;
    NSArray * splitArray = [self componentsSeparatedByString:@"."];
    NSString * hdImgName = [NSString stringWithFormat:@"%@@2x.%@",[splitArray objectAtIndexSafely:0],[splitArray objectAtIndexSafely:1]];
//    resPath = [resPath stringByAppendingPathComponent:(isHD)?hdImgName:self];
...
- (NSMutableDictionary *)dictionaryFromQueryComponents
{
    ...
    for(NSString *keyValuePairString in [self componentsSeparatedByString:@"&"])
    {
        ...
        if ([keyValuePairArray count] < 2)
            continue;
        NSString *key = [[keyValuePairArray objectAtIndexSafely:0] urlDecoding];
        NSString *value = [[keyValuePairArray objectAtIndexSafely:1] urlDecoding];
        if(value != nil) {
