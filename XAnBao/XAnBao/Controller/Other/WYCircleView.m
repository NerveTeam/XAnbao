...
//    KHWCircleLineWidth
    ...
    //半径
    CGFloat radius2 = (MIN(rect.size.width, rect.size.height) - KHWCircleLineWidth) * 0.5;
    //画弧（参数：中心、半径、起始角度(3点钟方向为0)、结束角度、是否顺时针）
    [path2 addArcWithCenter:(CGPoint){rect.size.width * 0.5, rect.size.height * 0.5} radius:radius2 startAngle:M_PI * 1.5 endAngle:M_PI * 1.5 + M_PI * 2 *1 clockwise:YES];
    //连线
    [path2 stroke];
