...
UIDocumentInteractionController *documentController4;
@interface XABOtherViewController ()<UITableViewDataSource,UITableViewDelegate,UIAlertViewDelegate,UIDocumentInteractionControllerDelegate>{
...
-(void)playMV:(NSString *)filename{
    ...
    if ([fileManager fileExistsAtPath:[cachePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", filename]]]) {
        ...
        documentController4.delegate = self;
        [documentController4 presentPreviewAnimated:YES];
...
- (UIView *)topBarView {
    if (!_topBarView) {
        ...
        _topBarView = [_topBarView topBarWithTintColor:ThemeColor title:@"其它" titleColor:[UIColor whiteColor] leftView:self.backBtn rightView:nil responseTarget:self];
