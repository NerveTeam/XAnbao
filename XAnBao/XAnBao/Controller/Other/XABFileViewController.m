...
UIDocumentInteractionController *documentController3;
@interface XABFileViewController ()<UITableViewDataSource,UITableViewDelegate,UIAlertViewDelegate,UIDocumentInteractionControllerDelegate>{
...
@implementation XABFileViewController
...
-(void)playMV:(NSString *)filename{
    ...
    if ([fileManager fileExistsAtPath:[cachePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", filename]]]) {
        ...
        [UIDocumentInteractionController
         interactionControllerWithURL:[NSURL fileURLWithPath:[cachePath stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", filename]]]];
        documentController3.delegate = self;
        [documentController3 presentPreviewAnimated:YES];
...
- (UIView *)topBarView {
    if (!_topBarView) {
        ...
        _topBarView = [_topBarView topBarWithTintColor:ThemeColor title:@"文件" titleColor:[UIColor whiteColor] leftView:self.backBtn rightView:nil responseTarget:self];
