//
//  XABSearchStudentViewController.m
//  XAnBao
//
...
@interface XABSearchStudentViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation XABSearchStudentViewController
...
- (void)search {
    ...
    [ClassSearchStudent requestDataWithParameters:pargam headers:Token successBlock:^(BaseDataRequest *request) {
...
- (void)enterClick:(UIButton *)sender {
    ...
    XABClassStudentDetailViewController *sd = [XABClassStudentDetailViewController new];
    sd.sid = [dic objectForKeySafely:@"id"];
    sd.classId = self.classId;
    [self pushToController:sd animated:YES];
}
...
- (UITextField *)inputPhoto {
    if (!_inputPhoto) {
        ...
        _inputPhoto.placeholder = @"输入学生姓名";
...
- (UIButton *)postBtn {
    if (!_postBtn) {
        _postBtn = [UIButton buttonWithTitle:@"查询学生" fontSize:15 titleColor:[UIColor whiteColor]];
